...
const TERM_CONFIG = {
  fall: { label: "Fall", key: "fall", start: { month: 8, day: 3 }, end: { month: 12, day: 18 } },
  spring: { label: "Spring", key: "spring", start: { month: 1, day: 5 }, end: { month: 5, day: 22 } }
};
...
const SUBJECT_DEFINITIONS = [
  ...
  { key: "it", label: "IT", colorVar: "var(--subject-it)", matches: ["information technology", "computer science", "computer", "it ", "web design", "programming", "cyber", "digital"] },
  { key: "health-science", label: "Health Science", colorVar: "var(--subject-health-science)", matches: ["health science", "healthcare", "nursing", "medical", "biotech", "therapeutic", "patient", "sports medicine"] },
  { key: "engineering", label: "Engineering", colorVar: "var(--subject-engineering)", matches: ["engineering", "robotics", "manufacturing", "aerospace", "drafting", "cad"] },
...
function loadState() {
  ...
  try {
    ...
    state.viewMode = parsed.viewMode === 'teacher' ? 'teacher' : 'admin';
...
function applyViewMode() {
  els.appShell.classList.toggle('admin-mode', state.viewMode === 'admin');
  els.appShell.classList.toggle('teacher-mode', state.viewMode === 'teacher');
  els.viewMode.value = state.viewMode;
}
...
function exportJSON() {
  const payload = JSON.stringify({ year: state.year, termFilter: state.termFilter, viewMode: state.viewMode, entries: state.entries }, null, 2);
...
function renderTimelineDescription(viewStart, viewEnd, label) {
  const modeText = state.viewMode === 'admin' ? 'Admin planning view' : 'Teacher cross-curricular view';
  els.timelineDescription.textContent = `${modeText} for ${label} from ${formatDate(viewStart)} to ${formatDate(viewEnd)}.`;
}
...
function inferSubject(courseText = '') {
  ...
  return SUBJECT_DEFINITIONS.find((subject) => subject.matches.some((term) => normalized.includes(` ${term} `) || normalized.includes(term))) || SUBJECT_DEFINITIONS.at(-1);
}
